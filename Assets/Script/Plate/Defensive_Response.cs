...
public class Defensive_Response : Plate
{
    ...
    protected override void Texting()
    {
        textLinkClickHandler.Texting
            ...
            " 피해를 입히고 모든 아군에게 보호막을 <color=#C54646><link=word>" + keywords[1].Word + "</link></color> 얻는다.");
    }

    //방어기능
    public override void Defense()
    {
        //모든 아군이 방어력을 keywords[1].Word 만큼 획득
        if (keywords[1].Word == 0)
            return;
        List<DefenceInfo> defenceInfos = new ();
        for(int i = 0; i < ally.Count; i++)
        {
            defenceInfos.Add(new DefenceInfo(ally[i], 0, keywords[1].Word));
        }
        //방어를 수행한 캐릭터에 속함
